...
public class GraphPanel extends JPanel
{
    ...
    public GraphPanel()
    {
        ...
        // Calling the print path button
        JButton printXY = new JButton("Print path X -> Y");
        printXY.addActionListener (new printXYListener());
        add(printXY);
    ...
    // The reason I made this a method and put it outside of the printListener, is so we can call it in another class
    // Now we can call it when we go to print our path!
    ...
    private class printListener implements ActionListener
    {
        ...
        {
            FillTree(); // Filling the tree with our current matrix
    ...
    public static ArrayList findPath(int x, int y, ArrayList path)
    {
        int[][] tree = a.clone(); // Cloning a because we're marking it so that there's no loops
        ...
        {   
            ...
            path1.add(y);
            System.out.println("EXIT BASE: " + path1);
            path2 = path1;
        }
        else 
        {
            int z;
            for (z=0; z<tree.length; z++)
                if (tree[x][z]==1) 
                {   
                    ArrayList path1 = (ArrayList)path.clone();
                    path1.add(z);
                    tree[x][z]=2;
                    path1 = findPath(z,y,path1);
    ...
    private class printXYListener implements ActionListener
    {
        public void actionPerformed (ActionEvent e)
        {
            int X = Integer.parseInt( JOptionPane.showInputDialog("Enter X Value:") ); // Prompting for the x value
            int Y = Integer.parseInt( JOptionPane.showInputDialog("Enter Y Value:") ); // Prompting for the y value
            
            FillTree(); // Filling the tree with the matrix that the user created. Calls on the FillTree method
            System.out.println( findPath(X,Y,path) ); // Printing out the results
